...
public class SplitFile {
    ...
    public static void myFunction(int lines, int files) throws FileNotFoundException, IOException
    {
        ...
        {
            FileWriter fstream1 = new FileWriter("FileAfterSplite/FileNumber_"+i+".csv"); //creating a new file writer.
            BufferedWriter out = new BufferedWriter(fstream1);
            for(int j=0;j<lines;j++)
            {   //iterating the reader to read only the first few lines of the csv as defined earlier
                strLine = br.readLine();
                if (strLine!= null)
                {
    ...
    public static void main(String args[])
    {
        try
        {
            ...
            {  //counting the lines in the input file
                scanner.nextLine();
                count++;
            }
